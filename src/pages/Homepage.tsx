...
export default function Home() {
  ...
  const [viewMode, setViewMode] = useState<"home" | "shortcut" | "menu">("home");
  ...
  return (
   ...
   {/* 전체 크기는 원래 큰 크기(h-8 w-8) 유지 */}
    ...
    {/* 2. 바탕이 되는 큰 흰색 원 (입체감을 위해 그림자 유지) */}
      ...
      {/* 🔵 3. 흰색 원 '정중앙'에 들어갈 아주 작은 파란색 알맹이 원 */}
      {/* h-3 w-3 (12px) 크기로 줄여서 흰색과의 크기 비율 차이를 극대화했습니다. */}
      ...
      {viewMode === "home" && (
        <>
          ...
          <div className="absolute top-12 left-0 right-0 px-[19px] z-10 flex gap-2.5 items-center justify-between">
            ...
            >
              <button type="submit" className="flex items-center justify-center">
                <img src="/Homepage/searchbar.svg" alt="Search Icon" className="w-5 h-5 cursor-pointer" />
              </button>
              <input 
                type="text" 
                value={searchQuery}
              ...
              />
              {searchQuery && (
                <button type="button" onClick={() => setSearchQuery("")} className="text-gray-400 hover:text-gray-600 text-sm font-bold px-1">
                  ✕
                </button>
              )}
            </form>
            ...
            <button 
              ...
              style={{ boxShadow: '0px 4px 4px 0px rgba(0, 0, 0, 0.1)' }}
            >
              <img src="/Homepage/road sign.svg" alt="Route Icon" className="w-[20px] h-[20px] mb-0.5" />
          ...
          <div className="absolute bottom-[100px] right-[19px] z-10 cursor-pointer">
            <button onClick={() => setViewMode("menu")} className="block active:scale-95 transition-all">
              <img src="/Homepage/Frame 49.svg" alt="Menu Open" className="w-[53px] h-[53px]" />
            </button>
          </div>
        </>
      )}
      ...
      {viewMode === "shortcut" && (
        <div className="absolute inset-0 z-10 flex flex-col justify-between bg-black/10 backdrop-blur-[1px]">
          <div className="bg-white px-4 pt-12 pb-4 shadow-md rounded-b-2xl">
            <button onClick={() => setViewMode("home")} className="text-sm text-[#4F80E2] mb-3 flex items-center gap-1 font-semibold">
              ← 지도 홈으로
            </button>
            <div className="flex flex-col gap-2">
              <div className="flex items-center gap-3 bg-[#F4F5F7] px-3 py-2.5 rounded-xl border border-gray-100">
                ...
                <input type="text" placeholder="출발지" defaultValue="대우관" className="bg-transparent outline-none text-sm font-medium w-full" />
              </div>
              <div className="flex items-center gap-3 bg-[#F4F5F7] px-3 py-2.5 rounded-xl border border-gray-100">
                <span className="w-2 h-2 rounded-full bg-orange-500"></span>
                <input type="text" placeholder="도착지" defaultValue="위당관" className="bg-transparent outline-none text-sm font-medium w-full" />
              </div>
            </div>
          </div>
            ...
            <div className="flex justify-between items-center">
              <div>
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-[#E65B5B] font-bold text-lg">5분 소요</span>
                  <span className="text-gray-400 text-sm">| 500m</span>
                </div>
                <p className="text-gray-700 font-semibold text-[15px]">대우관 → 위당관</p>
              </div>
